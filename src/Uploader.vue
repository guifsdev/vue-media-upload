<template>
  <div>
    ...
    <div>
      <div v-for="(image, index) in addedMedia" :key="'added-media-hidden-' + index" class="mu-mt-1">
        <input type="text" name="added_media[]" :value="image.name" hidden />
      </div>
      <div v-for="(image, index) in removedMedia" :key="'saved-media-hidden-' + index" class="mu-mt-1">
...
export default {
  ...
  emits: ["init", "change", "add", "remove", "max", "maxFilesize", "response"],
  components: {
    Loader,
  },
};
